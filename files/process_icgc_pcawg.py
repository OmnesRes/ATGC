...
icgc_maf = pd.read_csv(file_path / 'final_consensus_passonly.snv_mnv_indel.icgc.public.maf', sep='\t',
                        usecols=usecols,
                        low_memory=False)

icgc_maf = icgc_maf.loc[icgc_maf['Tumor_Sample_Barcode'].isin(pcawg_sample_table['aliquot_id'])]
icgc_maf.drop_duplicates(inplace=True)

sizes = dict(icgc_maf['Tumor_Sample_Barcode'].value_counts())
tumors_to_use = [i for i in sizes if sizes[i] < 200000]
icgc_maf = icgc_maf.loc[icgc_maf['Tumor_Sample_Barcode'].isin(tumors_to_use)]
...
icgc_maf['Ref'], icgc_maf['Alt'], icgc_maf['five_p'], icgc_maf['three_p'] = variant_features(icgc_maf)

with open(file_path / 'icgc_pcawg_maf.pkl', 'wb') as f:
    pickle.dump(icgc_maf, f)
