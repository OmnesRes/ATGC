#had to edit skopt/space/transformers.py, np.int32 lines 262, 275
import numpy as np
import pandas as pd
...
callbacks = [tf.keras.callbacks.EarlyStopping(monitor='val_weighted_CE', min_delta=0.01, patience=25, mode='min', restore_best_weights=True)]
...
with open(cwd / 'figures' / 'tumor_classification' / 'pcawg' / 'neural_net' / 'results' / 'context_weights.pkl', 'wb') as f:
...
with open(cwd / 'figures' / 'tumor_classification' / 'pcawg' / 'neural_net' / 'results' / 'context_predictions.pkl', 'wb') as f:
...
# 0.782944649888693 weighted acc
# 0.8079191238416176 unweighted acc
# 0.9797727898600691 auc