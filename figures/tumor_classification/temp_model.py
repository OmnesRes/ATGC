...
from model.new_KerasLayers import Activations, Ragged, Embed, StrandWeight, Dropout, Norm
...
class RaggedModels:
    ...
    class MIL_heads:
        ...
        def build(self):
            ...
            output_tensors = [[] for i in range(self.heads)]
            for index, i in enumerate(self.mil_hidden):
                if index == len(self.mil_hidden) - 1:
                    activation = None
                else:
                    activation = 'relu'
                for head in range(self.heads):
                    if index == 0:
                        output_tensors[head].append(tf.keras.layers.Dense(units=i, activation=activation)(fused[-1][:, head, :]))
                    else:
                        output_tensors[head].append(tf.keras.layers.Dense(units=i, activation=activation)(output_tensors[head][-1]))
                    if self.dropout and index != len(self.mil_hidden) - 1:
                        output_tensors[head].append(tf.keras.layers.Dropout(self.dropout)(output_tensors[head][-1]))
            ...
            #     else:
            #         activation = 'relu'
            #     layer = tf.keras.layers.Dense(units=i, activation=activation)
            #     for head in range(self.heads):
            #         if index == 0:
            #             output_tensors[head].append(layer(tf.concat([fused[-1][:, head, :], Embed(embedding_dimension=self.heads, trainable=False)(tf.constant(index, dtype=tf.int32, shape=(None, 1)))], axis=-1)))
            #         else:
            #             output_tensors[head].append(layer(output_tensors[head][-1]))
            #         if self.dropout and index != len(self.mil_hidden) - 1:
            #             output_tensors[head].append(tf.keras.layers.Dropout(self.dropout)(output_tensors[head][-1]))
            ...
            for output_type, output_dim, output_name in zip(self.output_types, self.output_dims, self.output_names):
                if output_type == 'quantiles':
                    ...
                    output_tensors = tf.concat([i[-1] for i in output_tensors], axis=-1)
