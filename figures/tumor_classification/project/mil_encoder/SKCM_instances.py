...
#                         cluster_order[4]: '#ffffff',
#                         cluster_order[5]: '#abdda4',
                        # cluster_order[6]: '#66c2a5',
                        # cluster_order[7]: '#3288bd'
                        # })
...
ax5.set_ylabel('Cluster', fontsize=16, rotation=270, labelpad=14)
...
# plt.savefig(cwd / 'figures' / 'tumor_classification' / 'project' / 'mil_encoder' / 'figures' / 'instance_figure_new.png', dpi=600)
...
##cluster logos
for cluster in np.arange(clusters)[np.argsort([np.median([i for i, j in zip(instance_attention, kmeans.labels_) if j == k]) for k in np.arange(clusters)])][::-1]:
