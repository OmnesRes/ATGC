...
for idx_train, idx_test in StratifiedKFold(n_splits=5, random_state=0, shuffle=True).split(y_strat, y_strat):
    ...
    with tf.device('/cpu:0'):
        ...
        ds_train_eval = tf.data.Dataset.from_tensor_slices(((
                                               five_p_loader_eval(idx_train),
                                               three_p_loader_eval(idx_train),
                                               ref_loader_eval(idx_train),
                                               alt_loader_eval(idx_train),
                                               strand_loader_eval(idx_train),
                                           ),
                                            (
                                                tf.gather(y_label, idx_train),
                                            ),
                                            tf.gather(y_weights, idx_train)
                                            ))
        ds_train_eval = ds_train_eval.shuffle(buffer_size=len(idx_train), reshuffle_each_iteration=True).batch(512, drop_remainder=False)

    for i in range(3):

        sequence_encoder = InstanceModels.VariantSequence(6, 4, 2, [16, 16, 16, 16], fusion_dimension=128)
        mil = RaggedModels.MIL(instance_encoders=[sequence_encoder.model], sample_encoders=[], heads=y_label.shape[-1], output_dims=[y_label.shape[-1]], mil_hidden=[256], attention_layers=[], dropout=.5, input_dropout=dropout, instance_dropout=.5, regularization=0)
        ...
        mil.model.fit(ds_train,
                      ...
                      epochs=20000,
                      validation_data=ds_train_eval,
                      validation_steps=len(idx_train) // 512 + 1,
                      callbacks=callbacks,
                      )
        run_eval = mil.model.evaluate(ds_train_eval)[-1]
...
with open(cwd / 'figures' / 'tumor_classification' / 'project' / 'mil_encoder' / 'results' / 'sequence_weights_no_val.pkl', 'wb') as f:
...
with open(cwd / 'figures' / 'tumor_classification' / 'project' / 'mil_encoder' / 'results' / 'sequence_predictions_no_val.pkl', 'wb') as f:
