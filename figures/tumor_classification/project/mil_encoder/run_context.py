...
index_loader = new_DatasetsUtils.Map.FromNumpytoIndices([j for i in indexes for j in i], dropout=.4)

five_p_loader = new_DatasetsUtils.Map.FromNumpyandIndices(five_p, tf.int16)
three_p_loader = new_DatasetsUtils.Map.FromNumpyandIndices(three_p, tf.int16)
ref_loader = new_DatasetsUtils.Map.FromNumpyandIndices(ref, tf.int16)
alt_loader = new_DatasetsUtils.Map.FromNumpyandIndices(alt, tf.int16)
strand_loader = new_DatasetsUtils.Map.FromNumpyandIndices(strand, tf.float32)

five_p_loader_eval = new_DatasetsUtils.Map.FromNumpy(five_p, tf.int16)
three_p_loader_eval = new_DatasetsUtils.Map.FromNumpy(three_p, tf.int16)
ref_loader_eval = new_DatasetsUtils.Map.FromNumpy(ref, tf.int16)
alt_loader_eval = new_DatasetsUtils.Map.FromNumpy(alt, tf.int16)
strand_loader_eval = new_DatasetsUtils.Map.FromNumpy(strand, tf.float32)
...
for idx_train, idx_test in StratifiedKFold(n_splits=5, random_state=0, shuffle=True).split(y_strat, y_strat):
    ...
    for i in range(1):
        ...
        mil = RaggedModels.MIL_heads(instance_encoders=[sequence_encoder.model], sample_encoders=[], heads=y_label.shape[-1], output_types=['other'], mil_hidden=[256, 1], attention_layers=[], dropout=.5, instance_dropout=.5, regularization=0, input_dropout=.4)
