import numpy as np
import pandas as pd
from model.new_Sample_MIL import InstanceModels, RaggedModels
...
gene_loader = new_DatasetsUtils.Map.FromNumpy(gene_counts, tf.float32)
...
for idx_train, idx_test in StratifiedKFold(n_splits=5, random_state=0, shuffle=True).split(y_strat, y_strat):
    ...
    ds_train = ds_train.apply(new_DatasetsUtils.Apply.StratifiedMinibatch(batch_size=batch_size, ds_size=len(idx_train)))
    ...
    for run in range(3):
        ...
        mil = RaggedModels.MIL(instance_encoders=[], sample_encoders=[gene_encoder.model], output_dims=[y_label.shape[-1]], output_types=['other'], mil_hidden=[512], mode='none', dropout=.5, final_regularization=0)
