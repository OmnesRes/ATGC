...
contexts_loader = new_DatasetsUtils.Map.FromNumpy(context_counts, tf.float32)
...
for idx_train, idx_test in StratifiedKFold(n_splits=5, random_state=0, shuffle=True).split(y_strat, y_strat):
    ...
    for run in range(3):
        ...
        # context_encoder = InstanceModels.Feature(shape=(context_counts.shape[-1]), input_dropout=0, layers=[512], regularization=.01)
