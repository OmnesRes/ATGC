...
ds_test = tf.data.Dataset.from_tensor_slices(((five_p_loader(idx_test),
                                                ...
                                                tf.gather(types, idx_test)),
                                                tf.gather(y_label, idx_test),
                                               ))
ds_test = ds_test.batch(len(idx_test), drop_remainder=False)

evaluations, histories, weights = pickle.load(open(cwd / 'figures' / 'controls' / 'samples' / 'sim_data' / 'sample_info' / 'experiment_1' / 'sample_model_dynamic_before.pkl', 'rb'))

sequence_encoder = InstanceModels.VariantSequence(6, 4, 2, [16, 16, 8, 8])
sample_encoder = InstanceModels.Type(shape=(), dim=len(np.unique(types)))

mil = RaggedModels.MIL(instance_encoders=[sequence_encoder.model], sample_encoders=[sample_encoder.model], pooling='dynamic', fusion='before')
...
with open(cwd / 'figures' / 'controls' / 'samples' / 'sim_data' / 'sample_info' / 'experiment_1' / 'sample_model_dynamic_before_eval.pkl', 'wb') as f:
