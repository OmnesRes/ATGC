...
evaluations, histories, weights = pickle.load(open(cwd / 'figures' / 'controls' / 'samples' / 'sim_data' / 'regression' / 'experiment_2' / 'sample_model_attention_mean.pkl', 'rb'))

predictions = []
attentions = []
for i in range(3):
    tile_encoder = InstanceModels.VariantSequence(6, 4, 2, [16, 16, 8, 8])
    mil = RaggedModels.MIL(instance_encoders=[tile_encoder.model], pooling='mean')
...
#
with open(cwd / 'figures' / 'controls' / 'samples' / 'sim_data' / 'regression' / 'experiment_2' / 'sample_model_attention_mean_predictions.pkl', 'wb') as f:
