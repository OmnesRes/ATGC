...
classes = np.array([D['class'][i] for i in indexes], dtype='object')
...
for i in range(3):
    ...
    mil = RaggedModels.MIL(instance_encoders=[tile_encoder.model], pooling='dynamic', regularization=.05)
    ...
    callbacks = [tf.keras.callbacks.EarlyStopping(monitor='val_binary_crossentropy', min_delta=0.00001, patience=40, mode='min', restore_best_weights=True)]
    history = mil.model.fit(ds_train, steps_per_epoch=10, validation_data=ds_valid, epochs=10000, callbacks=callbacks)
    valid_evaluation = mil.model.evaluate(ds_valid)
...
with open(cwd / 'figures' / 'controls' / 'samples' / 'sim_data' / 'classification' / 'experiment_2' / 'sample_model_attention_dynamic.pkl', 'wb') as f:
