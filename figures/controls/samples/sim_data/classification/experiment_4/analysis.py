...
attentions = []
evaluations, histories, weights = pickle.load(open(cwd / 'figures' / 'controls' / 'samples' / 'sim_data' / 'classification' / 'experiment_4' / 'sample_model_attention_dynamic.pkl', 'rb'))
for i in range(3):
    mil.model.set_weights(weights[i])
    attentions.append(mil.attention_model.predict(ds_test).to_list())

with open(cwd / 'figures' / 'controls' / 'samples' / 'sim_data' / 'classification' / 'experiment_4' / 'dynamic_attentions.pkl', 'wb') as f:
