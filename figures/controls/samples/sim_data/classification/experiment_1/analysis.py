...
mil = RaggedModels.MIL(instance_encoders=[tile_encoder.model], pooling='mean', regularization=.05)
attentions = []
evaluations, histories, weights = pickle.load(open(cwd / 'figures' / 'controls' / 'samples' / 'sim_data' / 'classification' / 'experiment_1' / 'sample_model_attention_mean.pkl', 'rb'))
for i in range(3):
    mil.model.set_weights(weights[i])
    attentions.append(mil.attention_model.predict(ds_test).to_list())

with open(cwd / 'figures' / 'controls' / 'samples' / 'sim_data' / 'classification' / 'experiment_1' / 'mean_attentions.pkl', 'wb') as f:
