...
sample_sum_evaluations, sample_sum_histories, weights = pickle.load(open(cwd / 'figures' / 'controls' / 'samples' / 'sim_data' / 'survival' / 'experiment_2' / 'sample_model_attention_dynamic.pkl', 'rb'))
...
for index, (idx_train, idx_test) in enumerate(StratifiedKFold(n_splits=5, random_state=0, shuffle=True).split(y_strat, y_strat)):
    ...
    tile_encoder = InstanceModels.VariantSequence(6, 4, 2, [16, 16, 8, 8], fusion_dimension=128)
    mil = RaggedModels.MIL(instance_encoders=[tile_encoder.model], pooling='dynamic', mil_hidden=[128, 64, 32], regularization=.05, dropout=.1, instance_dropout=.3)
...
with open(cwd / 'figures' / 'controls' / 'samples' / 'sim_data' / 'survival' / 'experiment_2' / 'sample_model_attention_dynamic_eval.pkl', 'wb') as f:
    pickle.dump([indexes, ranks, sample_dfs, attentions], f)
