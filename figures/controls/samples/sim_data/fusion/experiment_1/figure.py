...
instance_sum_evaluations, instance_sum_histories, weights = pickle.load(open(cwd / 'figures' / 'controls' / 'samples' / 'sim_data' / 'fusion' / 'experiment_1' / 'no_instance_layers.pkl', 'rb'))
instance_mean_evaluations, instance_mean_histories, weights = pickle.load(open(cwd / 'figures' / 'controls' / 'samples' / 'sim_data' / 'fusion' / 'experiment_1' / 'instance_layers.pkl', 'rb'))


losses = np.array([i[-1] for i in instance_sum_evaluations + instance_mean_evaluations
                 ])
losses = losses / max(losses)


metrics = [-i[1] for i in instance_sum_evaluations + instance_mean_evaluations
                   ]
...
centers = np.concatenate([np.arange(3) + i * 3.2 for i in range(2)])
fig = plt.figure()
