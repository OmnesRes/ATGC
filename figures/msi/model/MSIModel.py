...
class InputFeatures:
    ...
    class VariantSequence:
        def __init__(self, sequence_length, sequence_embedding_dimension, n_strands, convolution_params, data, name='variant_sequence', default_activation=tf.keras.activations.relu, use_frame=False):
            self.name = name
        ...
        def encode(self, input_tensors):
            ...
            fused = tf.keras.layers.Dense(units=64, activation=self.default_activation, kernel_regularizer=tf.keras.regularizers.l2())(fused)
...
class ATGC:
    def __init__(self, features, latent_dimension):
    ...
    def build_mil_model(self, mil_hidden=(64, 32), output_dim=1, output_extra=0, output_type='classification_probability', aggregation='recursion'):
        ...
        sample_logits = self.mil_top(tf.concat(sample_aggregations, axis=-1), hidden_units=mil_hidden, logits_units=output_dim)
        ...
        self.mil_model = tf.keras.Model(inputs=[instance_sample_idx] + instance_features + [sample_idx, samples_n], outputs=[sample_output])
        self.intermediate_model = tf.keras.Model(inputs=[instance_sample_idx] + instance_features + [sample_idx, samples_n], outputs=[attentions])