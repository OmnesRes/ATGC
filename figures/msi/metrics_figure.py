...
with open(cwd / 'figures' / 'msi' / 'results' / 'mil_scores.pkl', 'rb') as f:
    mil_recalls, mil_precisions, mil_scores, mil_predictions, samples, y_label, test_idx = pickle.load(f)
...
sns.heatmap(precision_matrix_normed, annot=np.around(precision_matrix * 100).astype(np.int32), vmin=vmin, vmax=vmax, cmap=myblue, ax=ax1, cbar=False, fmt='d', annot_kws={'fontsize': 14}, square=True)
...
ax1.set_xticks(np.array(range(3)) + .5)
ax1.set_xticklabels(['UCEC', 'STAD', 'COAD'])
...
myblue = make_colormap({vmin: '#ffffff', vmax: '#4169E1'})
sns.heatmap(recall_matrix_normed, annot=np.around(recall_matrix * 100).astype(np.int32), vmin=vmin, vmax=vmax, cmap=myblue, ax=ax2, cbar=False, fmt='d', annot_kws={'fontsize': 14}, square=True)
ax2.tick_params(axis='x', length=0, width=0, labelsize=12, rotation=270)
ax2.set_title('Recalls', fontsize=18)
ax2.set_yticks([])
ax2.set_xticks(np.array(range(3)) + .5)
ax2.set_xticklabels(['UCEC', 'STAD', 'COAD'])
