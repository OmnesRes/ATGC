...
D, samples, maf, sample_df = pickle.load(open(cwd / 'figures' / 'tmb' / 'pcawg' / 'VICC_01_R2' / 'data' / 'data_cds.pkl', 'rb'))
...
for idx_train, idx_test in StratifiedKFold(n_splits=8, random_state=0, shuffle=True).split(y_strat, y_strat):
    ...
    ds_train = ds_train.apply(DatasetsUtils.Apply.StratifiedMinibatch(batch_size=128, ds_size=len(idx_train)))
...
with open(cwd / 'figures' / 'tmb' / 'pcawg' / 'VICC_01_R2' / 'results' / 'run_naive_cds.pkl', 'wb') as f:
