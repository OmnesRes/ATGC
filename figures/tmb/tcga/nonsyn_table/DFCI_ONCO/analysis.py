...
all_weights = [pickle.load(open(cwd / 'figures' / 'tmb' / 'tcga' / 'nonsyn_table' / 'DFCI_ONCO' / 'results' / 'run_naive.pkl', 'rb')),
            ]
...
results['counting'] = np.log(sample_df['panel_all_counts'].values[np.concatenate(test_idx)] / (panels.loc[panels['Panel'] == 'DFCI-ONCOPANEL-3']['cds'].values[0]/1e6) + 1)
