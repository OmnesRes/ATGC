...
types_loader = DatasetsUtils.Map.FromNumpy(types_onehot, tf.float32)
...
callbacks = [tf.keras.callbacks.EarlyStopping(monitor='val_coxph', min_delta=0.0001, patience=100, mode='min', restore_best_weights=True)]

initial_weights = []
for i in range(3):
    context_encoder = InstanceModels.Feature(shape=(context_counts.shape[-1]), input_dropout=0.4, layers=[64, 16], regularization=0, layer_dropouts=[.2, 0])
    types_encoder = InstanceModels.PassThrough(shape=(types_onehot.shape[-1]))
    mil = RaggedModels.MIL(instance_encoders=[], sample_encoders=[context_encoder.model, types_encoder.model], output_dims=[1], mil_hidden=[256, 128, 64], mode='none', dropout=.1)
...
for idx_train, idx_test in StratifiedKFold(n_splits=10, random_state=0, shuffle=True).split(y_strat, y_strat):
    ...
    ds_train = ds_train.apply(DatasetsUtils.Apply.StratifiedMinibatch(batch_size=int(len(idx_train) * .5), ds_size=len(idx_train)))
    ...
    for run in range(3):
        mil.model.set_weights(initial_weights[run])
...
with open(cwd / 'figures' / 'survival' / 'results' / 'context_weights.pkl', 'wb') as f:
