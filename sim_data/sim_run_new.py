...
mil = RaggedModels.MIL(instance_encoders=[tile_encoder.model], sample_encoders=[], output_dim=2, regularization=.1, output_type='other')

losses = [tf.keras.losses.CategoricalCrossentropy(from_logits=True)]
mil.model.compile(loss=losses, metrics=['accuracy', tf.keras.metrics.CategoricalCrossentropy(from_logits=True)])
