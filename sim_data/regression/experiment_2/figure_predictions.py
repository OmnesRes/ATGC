...
y_true = np.log(np.array(samples['values'])[idx_test] + 1)
...
for index, i in enumerate(['True Value', 'Instance Sum', 'Instance Mean', 'Sample Sum', 'Sample Mean']):
    if index != 0:
        for index2, (x, y) in enumerate(zip(x_true, predictions[index][0])):
...
ax.set_yticks([])
ax.set_xticks([])
ax.set_ylabel('Bag Value (log scale)', fontsize=24, labelpad=-10)
