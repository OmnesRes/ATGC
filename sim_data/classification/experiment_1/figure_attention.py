...
classes = np.concatenate([np.concatenate(classes), np.concatenate(classes)])
attention = np.concatenate([np.concatenate(mean_attentions[2][:20]) - min(np.concatenate(mean_attentions[2][:20])), np.concatenate(sum_attentions[2][:20]) - min(np.concatenate(sum_attentions[2][:20]))])
...
ax.legend(frameon=False, title='Model', fontsize=10, title_fontsize=12, loc=(-.05, .95), ncol=3)
