...
def generate_sample(mean_variants=[300],
    ...
    total_count = int(np.random.normal(center, int(np.ceil(center * .01))))
    if total_count < 1:
        total_count *= -1
    if total_count == 0:
...
for idx in range(1000):
    ...
    if np.random.sample() < .5:
        ...
        variants = generate_sample(control=False, num_positive=1, positive_choices=positive_choices)
...
with open(cwd / 'sim_data' / 'attention' / 'experiment_1' / 'sim_data.pkl', 'wb') as f:
